...
public class SysUserService extends ServiceImpl<SysUserMapper, SysUser> {
  ...
  public void register(String nickName, String username, String password) {
    ...
    updateObj.setId(id);
    updateObj.setPasswordSalt(SaSecureUtil.sha256(id.toString()));
    updateObj.setPassword(SaSecureUtil.md5BySalt(password, updateObj.getPasswordSalt()));
  ...
  public SysUser login(String username, String password) {
    ...
    if (!user.getPassword().equals(SaSecureUtil.md5BySalt(password, user.getPasswordSalt()))) {
      throw new ServiceException(HttpStatus.UNAUTHORIZED, "用户名或密码错误");
    }
